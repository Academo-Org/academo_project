...
// Busca todos os alunos e professores
$users = $conn->query("SELECT id_users, name, email, login, role_id, status FROM users WHERE role_id IN (2, 3) ORDER BY name")->fetch_all(MYSQLI_ASSOC);
?>

<style>
    /* Estilo para alinhar os botões de ação */
    .action-buttons {
        display: flex;
